...
void analys_door_eye_protocol(unsigned char *data)
{
...
void analys_data_packet(DOOR_EYE_PROTOCOL_DATA *protocol_data)
{
    switch (protocol_data->cmd_code)
    {
        case GET_TIMESTAMP://获取时间（用于锁的日期时间调整）
        {
            ...
            data_copy(&tx_protocol.packet_data[0], (unsigned char*)&current_date, 6);
        }
        break;
        ...
        case VISUAL_DOORBELL_STATUS://获取可视模块当前状态 
        {
            ...
            data_copy(&tx_protocol.packet_data[0], (unsigned char*)&door_State, 1);
...
void data_package(unsigned char PID)
{
	...
	tx_buff[1] = 0xCD;
	tx_buff[2] = PID;
	tx_buff[3] = tx_protocol.data_len;
	tx_buff[4] = 0;
	tx_buff[5] = tx_protocol.cmd_code;
    data_copy(&tx_buff[6U], (unsigned char *)&tx_protocol.packet_data[0U], tx_protocol.packet_data_len);
    tx_protocol.check_sum = count_checksum(&tx_buff[2], tx_protocol.packet_data_len + 4);
    ...
    tx_buff[tx_protocol.packet_data_len + 10] = '\0';
...
void send_serial_data(unsigned char *data)
{
   ...
   for (i = 0; i < (tx_protocol.packet_data_len + 10); i++)
	{
        //printf("%x", data[i]);
		send_data_to_serial(data[i]);
   	}
}
